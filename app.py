...
def inject_whalewatch_theme() -> None:
    st.markdown(
        ...
        .ww-simple-brand-title {
            font-size: 1.42rem;
            line-height: 1;
            font-weight: 900;
        }

        .ww-simple-brand-subtitle {
            color: var(--ww-muted);
            font-size: 0.74rem;
            font-weight: 800;
        ...
        .ww-page-icon {
            ...
            border-radius: 14px;
            color: var(--ww-green);
            border: 1px solid rgba(32, 242, 156, 0.24);
            background: rgba(32, 242, 156, 0.10);
            font-size: 1.28rem;
            font-weight: 900;
        }
        ...
        .ww-trader-card, .ww-simple-panel, .ww-buy-card, .ww-alert-simple {
            border: 1px solid rgba(116, 147, 185, 0.16);
            background: linear-gradient(145deg, rgba(13, 18, 28, 0.95), rgba(6, 10, 17, 0.95));
            border-radius: 14px;
        ...
        .ww-trader-card:hover {
            transform: translateY(-2px);
            border-color: rgba(32, 242, 156, 0.34);
            background: linear-gradient(145deg, rgba(13, 23, 31, 0.98), rgba(6, 11, 18, 0.96));
        }
        ...
        .ww-trade-icon.sell {
            color: var(--ww-red);
            background: rgba(255, 82, 109, 0.10);
        }
        ...
        @media (max-width: 1100px) {
            .ww-card-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
        }
        ...
        @media (max-width: 820px) {
            ...
            .ww-buy-card, .ww-alert-simple, .ww-watch-row { align-items: flex-start; flex-direction: column; }
        }
        </style>
        """,
...
def crypto_watchlist_metric_payload(row: dict | None, last_activity: str | None = None) -> dict:
    ...
    display_label = firestore_scalar(row.get("display_label") or row.get("name") or f"{tier} wallet")
    payload = {
        ...
        "estimated_profit": crypto_estimated_profit_value(row),
        "roi_pct": roi,
        "roi": roi,
        ...
        "added_at": firestore_scalar(row.get("added_at")),
    }
...
def add_crypto_wallet_to_watchlist(wallet: str, row: dict | None = None) -> None:
    ...
    if enriched_row:
        enriched_row.setdefault("added_at", time.strftime("%Y-%m-%d %H:%M UTC", time.gmtime()))
        enriched_row.setdefault("display_label", f"{normalize_tier_name(enriched_row.get('whale_tier'))} wallet")
    metadata = crypto_watchlist_metric_payload(enriched_row, enriched_row.get("last_activity") if enriched_row else None)
    try:
        saved_items = store.upsert_watchlist(
            str(session["id_token"]),
            str(session["user_id"]),
            wallet,
            wallet_label=normalize_tier_name(enriched_row.get("whale_tier")) if enriched_row else None,
            collection="crypto_watchlist",
            metadata=metadata,
        )
    except FirebaseError:
...
def persist_crypto_watchlist_metrics(wallet: str, row: dict, last_activity: str | None = None) -> dict:
    ...
    existing_item = existing.get(wallet, {"wallet": wallet})
    has_valid_live_metrics = any(
        optional_metric_number(metadata.get(key)) is not None
        for key in ["whale_score", "total_volume", "avg_trade_size", "profitable_trade_pct", "roi_pct"]
    )
    if not has_valid_live_metrics:
        if last_activity:
            metadata = {"last_activity": firestore_scalar(last_activity)}
        else:
            return {}
    saved_tier = normalize_tier_name(existing_item.get("whale_tier") or existing_item.get("tier") or existing_item.get("wallet_label"))
    new_tier = normalize_tier_name(metadata.get("whale_tier"))
    if saved_tier != "Dolphin" and new_tier == "Dolphin" and safe_number(metadata.get("whale_score")) <= 0:
        metadata["wallet_label"] = saved_tier
        metadata["whale_tier"] = saved_tier
        metadata["tier"] = saved_tier
    if existing_item.get("added_at") and not metadata.get("added_at"):
        metadata["added_at"] = existing_item.get("added_at")
    if existing_item.get("display_label") and not metadata.get("display_label"):
        metadata["display_label"] = existing_item.get("display_label")
    session = current_auth_session()
    store = get_firebase_store()
...
def relative_time(timestamp_raw) -> str:
    timestamp = crypto_timestamp_to_raw(timestamp_raw)
    if not timestamp:
        return "recently"
...
def simple_page_header(icon: str, title: str, subtitle: str) -> None:
    st.markdown(
        f"""
        <div class="ww-page-hero">
          <div class="ww-page-icon">{html.escape(icon)}</div>
          <div>
            <div class="ww-page-title">{html.escape(title)}</div>
            <div class="ww-page-subtitle">{html.escape(subtitle)}</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
@st.cache_data(ttl=5 * 60, show_spinner=False)
def cached_crypto_activity_events_for_display(
    wallet: str,
    chain: str,
    api_key: str | None,
    token_filter: str,
    ...
    activity = fetch_crypto_recent_trades(wallet, chain, api_key, token_filter, int(time_period_days))
    return build_crypto_activity_events_resilient(wallet, activity, chain)


def simple_activity_for_wallets(
    wallets: list[str],
    chain: str,
    token_filter: str,
    time_period_days: int,
    ...
    for wallet in wallets:
        ...
        if len(unique_wallets) >= max_wallets:
            break
    for wallet in unique_wallets:
        try:
            events = cached_crypto_activity_events_for_display(wallet, chain, api_key, token_filter, int(time_period_days))
        except CryptoAPIError:
            logger.warning("Could not load beginner activity for %s", wallet, exc_info=True)
            continue
        if not events:
            continue
        frame = pd.DataFrame(events)
        frame["wallet"] = wallet
        frame["wallet_label"] = crypto_wallet_label_text(wallet, ranked_lookup)
        frames.append(frame)
    return enrich_crypto_activity_display(pd.concat(frames, ignore_index=True)) if frames else pd.DataFrame()


def friendly_crypto_trades(activity_df: pd.DataFrame, buys_only: bool = False) -> pd.DataFrame:
    activity_df = enrich_crypto_activity_display(activity_df)
    if activity_df.empty:
        return activity_df
    for column, default in {"category": "", "action": "", "dollar_value": 0.0}.items():
        if column not in activity_df.columns:
            activity_df[column] = default
    trades = activity_df[
        (activity_df["category"].astype(str) == "Trades / Swaps")
    ...
    if buys_only:
        trades = trades[trades["action"].astype(str).str.upper() == "BUY"]
    if trades.empty:
        return trades
    trades["dollar_value"] = pd.to_numeric(trades["dollar_value"], errors="coerce").fillna(0.0)
    return safe_sort_crypto_activity(trades)


def main_tokens_from_trades(trades_df: pd.DataFrame, limit: int = 4) -> list[str]:
    trades_df = friendly_crypto_trades(trades_df)
    if trades_df.empty:
        return []
    tokens: list[str] = []
    for _, row in trades_df.iterrows():
        action = str(row.get("action") or "").upper()
        token = row.get("token_bought") if action == "BUY" else row.get("token_sold") or row.get("token_bought")
        token = str(token or "").upper()
        if token and token != "UNKNOWN" and token not in tokens:
            tokens.append(token)
        if len(tokens) >= limit:
            break
    return tokens
...
def simple_whale_label(row: dict) -> str:
    label = str(row.get("display_label") or "").strip()
    if label and label.lower() not in {"dolphin", "shark", "blue whale", "leviathan", "kraken"}:
        return label
    return f"{normalize_tier_name(row.get('whale_tier'))} wallet"


def simple_ranked_lookup(rows: list[dict]) -> dict[str, dict]:
    lookup = {str(row.get("wallet", "")).lower(): row for row in rows if row.get("wallet")}
    for item in st.session_state.get("crypto_watchlist_items", []):
        wallet = str(item.get("wallet", "")).lower()
        if wallet:
            lookup[wallet] = {**item, **lookup.get(wallet, {})}
    return lookup


def simple_card_tokens_html(tokens: list[str]) -> str:
    if not tokens:
        return '<span class="ww-token-pill">Watching activity</span>'
    return "".join(f'<span class="ww-token-pill">{html.escape(token)}</span>' for token in tokens[:4])


def simple_trader_card_html(row: dict, activity_df: pd.DataFrame, ranked_lookup: dict[str, dict]) -> str:
    wallet = str(row.get("wallet") or "").lower()
    tier = crypto_wallet_tier(wallet, ranked_lookup)
    name = simple_whale_label({**row, "whale_tier": tier})
    profit = format_optional_money(crypto_estimated_profit_value(row), "Estimating")
    profitable = format_optional_percent(crypto_profitable_trade_pct_value(row))
    recent_buys = len(friendly_crypto_trades(activity_df, buys_only=True))
    tokens = main_tokens_from_trades(activity_df)
    initial = html.escape((name.strip()[:1] or "W").upper())
    return f"""
    <div class="ww-trader-card">
      <div class="ww-card-top">
        <div style="display:flex; gap:0.85rem; min-width:0;">
          <div class="ww-avatar">{initial}</div>
          <div style="min-width:0;">
            <div style="margin-bottom:0.35rem;">{tier_html(tier)}</div>
            <div class="ww-whale-name">{html.escape(name)}</div>
            <div class="ww-small-address">{html.escape(short_wallet(wallet))}</div>
          </div>
        </div>
      </div>
      <div class="ww-card-metrics">
        <div class="ww-mini-metric"><div class="ww-mini-label">Estimated Profit</div><div class="ww-mini-value ww-num-positive">{html.escape(profit)}</div></div>
        <div class="ww-mini-metric"><div class="ww-mini-label">Profitable Trade %</div><div class="ww-mini-value">{html.escape(profitable)}</div></div>
        <div class="ww-mini-metric"><div class="ww-mini-label">Recent Buys</div><div class="ww-mini-value">{recent_buys}</div></div>
      </div>
      <div class="ww-token-row">{simple_card_tokens_html(tokens)}</div>
    </div>
    """


def render_simple_trade_cards(
    trades_df: pd.DataFrame,
    ranked_lookup: dict[str, dict],
    title: str,
    empty_title: str,
    ...
    for _, row in trades_df.head(limit).iterrows():
        action = str(row.get("action") or "").upper()
        wallet = str(row.get("wallet") or "").lower()
        token = str(row.get("token_bought") if action == "BUY" else row.get("token_sold") or row.get("token_bought") or "TOKEN").upper()
        value = safe_number(row.get("dollar_value"))
        action_word = "bought" if action == "BUY" else "sold" if action == "SELL" else "swapped"
        icon = "^" if action == "BUY" else "v" if action == "SELL" else ">"
        icon_class = "" if action != "SELL" else " sell"
        link = str(row.get("explorer_link") or "")
        link_html = f'<a href="{html.escape(link)}" target="_blank">Transaction</a>' if link else ""
        st.markdown(
            f"""
            <div class="ww-buy-card">
              <div class="ww-buy-left">
                <div class="ww-trade-icon{icon_class}">{icon}</div>
                <div>
                  <div class="ww-buy-title">{crypto_wallet_badge_html(wallet, ranked_lookup)} {html.escape(action_word)} {format_money(value)} of {html.escape(token)}</div>
                  <div class="ww-buy-meta">{html.escape(relative_time(row.get("timestamp_raw")))} {link_html}</div>
                </div>
              </div>
            </div>
            """,
...
def render_simple_popular_buys(
    activity_df: pd.DataFrame,
    ranked_lookup: dict[str, dict],
    key_prefix: str,
    title: str = "Popular Buys",
) -> None:
    render_compact_title(title)
    filter_cols = st.columns([1, 1, 3])
    window_label = filter_cols[0].selectbox("Time", ["Last 24h", "Last 7d", "Last 30d"], index=1, key=f"{key_prefix}-simple-window")
    min_buy = filter_cols[1].number_input("Minimum buy", min_value=0.0, value=500.0, step=250.0, key=f"{key_prefix}-simple-min-buy")
    days = {"Last 24h": 1, "Last 7d": 7, "Last 30d": 30}[window_label]
    popular = recently_bought_tokens_dataframe(activity_df, float(min_buy), days, ranked_lookup)
    if popular.empty:
        render_empty_state("No popular buys yet", "No watched whales bought tokens above this size in the selected window.")
        return
    for _, row in popular.head(6).iterrows():
        st.markdown(
            f"""
            <div class="ww-watch-row">
              <div>
                <div class="ww-whale-name">{html.escape(str(row.get("token") or "TOKEN"))}</div>
                <div class="ww-buy-meta">{format_money(row.get("total_bought_recently"))} bought by {int(safe_number(row.get("whale_buyers")))} watched wallet(s)</div>
              </div>
              <div class="ww-buy-meta" style="text-align:right;">
                Largest buy {format_money(row.get("largest_buy_size"))}<br>{html.escape(str(row.get("last_buy_time") or ""))}
              </div>
            </div>
            """,
            unsafe_allow_html=True,
...
def render_simple_wallet_details(
    wallet: str,
    metadata: dict,
    chain: str,
    token_filter: str,
    ...
    if not wallet:
        return
    details = {**metadata, "wallet": wallet}
    activity_df = simple_activity_for_wallets([wallet], chain, token_filter, int(time_period_days), ranked_lookup, max_wallets=1)
    watched = wallet in st.session_state.get("crypto_watchlist", [])
    st.markdown(
        f"""
        <div class="ww-simple-panel">
          <div class="ww-card-top">
            <div>
              <div style="margin-bottom:0.5rem;">{crypto_wallet_badge_html(wallet, ranked_lookup)}</div>
              <div class="ww-small-address">{html.escape(wallet)}</div>
            </div>
            <div class="ww-token-row">
              <span class="ww-token-pill">Profitable Trade %: {html.escape(format_optional_percent(crypto_profitable_trade_pct_value(details)))}</span>
              <span class="ww-token-pill">Estimated ROI: {html.escape(format_optional_percent(crypto_estimated_roi_value(details)))}</span>
            </div>
          </div>
        </div>
        """,
    ...
    if watched:
        if action_cols[0].button("Unwatch", key=f"simple-unwatch-{wallet}"):
            remove_crypto_wallet_from_watchlist(wallet)
            st.rerun()
    else:
        if action_cols[0].button("Watch", type="primary", key=f"simple-watch-detail-{wallet}"):
            add_crypto_wallet_to_watchlist(wallet, details)
            st.rerun()
    if action_cols[1].button("Close", key=f"simple-close-detail-{wallet}"):
        st.session_state["simple_selected_wallet"] = ""
        st.rerun()
    render_simple_trade_cards(
        friendly_crypto_trades(activity_df, buys_only=True),
        ranked_lookup,
        "Recent Buys",
        "No recent buys",
        "This wallet has no decoded public buys in the selected window.",
        limit=5,
    )


def render_simple_leaderboard(
    rows: list[dict],
    chain: str,
    token_filter: str,
    time_period_days: int,
    ...
    ensure_crypto_watchlist_state()
    simple_page_header("T", "Whale Leaderboard", "Simple cards showing whales worth watching.")
    if not rows:
        render_empty_state("No whales loaded", "Use Find Whales in the sidebar to scan public wallet activity.")
        return
    rows = sorted(rows, key=lambda row: safe_number(row.get("whale_score")), reverse=True)
    ranked_lookup = simple_ranked_lookup(rows)
    top_rows = rows[:12]
    activity_df = simple_activity_for_wallets(
        [str(row.get("wallet") or "") for row in top_rows],
        chain,
        token_filter,
        int(time_period_days),
        ranked_lookup,
    ...
    for index, row in enumerate(top_rows):
        wallet = str(row.get("wallet") or "").lower()
        wallet_activity = activity_df[activity_df["wallet"].astype(str).str.lower() == wallet] if not activity_df.empty else pd.DataFrame()
        with cols[index % 3]:
            st.markdown(simple_trader_card_html(row, wallet_activity, ranked_lookup), unsafe_allow_html=True)
            button_cols = st.columns([1, 1])
            watched = wallet in st.session_state.get("crypto_watchlist", [])
            if button_cols[0].button("Watching" if watched else "Watch", disabled=watched, key=f"simple-watch-{wallet}"):
                add_crypto_wallet_to_watchlist(wallet, row)
                st.rerun()
            if button_cols[1].button("Details", key=f"simple-details-{wallet}"):
                st.session_state["simple_selected_wallet"] = wallet
                st.rerun()
    selected = str(st.session_state.get("simple_selected_wallet") or "").lower()
    if selected:
        selected_row = next((row for row in rows if str(row.get("wallet") or "").lower() == selected), {})
        render_simple_wallet_details(selected, selected_row, chain, token_filter, int(time_period_days), ranked_lookup)


def render_simple_watchlist(chain: str, token_filter: str, time_period_days: int, min_transaction_size: float, rows: list[dict]) -> None:
    ensure_crypto_watchlist_state()
    ranked_lookup = simple_ranked_lookup(rows)
    simple_page_header("O", "Your Watchlist", "Whales you're following and what they just bought.")
    if not st.session_state["crypto_watchlist_items"]:
        render_empty_state("No watched whales yet", "Add whales from the leaderboard to start tracking their buys.")
        return
    if st.button("Refresh Watchlist", key="simple-refresh-watchlist"):
        cached_crypto_activity_events_for_display.clear()
        cached_crypto_recent_trades.clear()
        sync_user_data_from_firebase(force=True)
        st.rerun()
    activity_df = simple_activity_for_wallets(
    ...
    last_times = last_trade_times_from_df(activity_df.rename(columns={"dollar_value": "value"})) if not activity_df.empty else {}
    hydrate_crypto_watchlist_metrics(chain, token_filter, int(time_period_days), float(min_transaction_size), ranked_lookup, last_times)
    render_compact_title("Saved Wallets")
    for item in list(st.session_state["crypto_watchlist_items"]):
        wallet = str(item.get("wallet") or "").lower()
        merged = {**item, **ranked_lookup.get(wallet, {})}
        st.markdown(
            ...
            <div class="ww-watch-row">
              <div>
                <div>{crypto_wallet_badge_html(wallet, ranked_lookup)}</div>
                <div class="ww-buy-meta">
                  Estimated Profit {html.escape(format_optional_money(crypto_estimated_profit_value(merged), "Estimating"))}
                  &nbsp; | &nbsp; Profitable Trade % {html.escape(format_optional_percent(crypto_profitable_trade_pct_value(merged)))}
                </div>
              </div>
            </div>
            """,
        ...
        if st.button("Remove", key=f"simple-remove-{wallet}"):
            remove_crypto_wallet_from_watchlist(wallet)
            st.rerun()
    render_simple_trade_cards(
        friendly_crypto_trades(activity_df, buys_only=True),
        ranked_lookup,
        "Latest Buys From Your Watchlist",
        "No recent buys",
        "Watched whales have not made decoded token buys in this window.",
        limit=8,
    )
    render_simple_popular_buys(activity_df, ranked_lookup, "watchlist-popular", "Popular Buys From Your Watchlist")


def render_simple_alerts(chain: str, token_filter: str, time_period_days: int, min_trade_value: float, rows: list[dict]) -> None:
    ensure_crypto_watchlist_state()
    ranked_lookup = simple_ranked_lookup(rows)
    simple_page_header("!", "Alerts", "Simple buy alerts from whales you watch.")
    if not st.session_state["crypto_watchlist"]:
        render_empty_state("No alerts yet", "Add whales to your watchlist first.")
        return
    activity_df = simple_activity_for_wallets(
        st.session_state["crypto_watchlist"],
        chain,
    ...
    detect_crypto_alerts(activity_df, float(min_trade_value), ranked_lookup)
    alerts = st.session_state.get("crypto_new_trade_alerts", [])
    if st.button("Clear Alerts", key="simple-clear-alerts"):
        st.session_state["crypto_new_trade_alerts"] = []
        st.rerun()
    if not alerts:
        render_simple_trade_cards(
            ...
            "Recent Watched Buys",
            "No new alerts",
            "New watched-wallet buys above your alert size will show here.",
            limit=8,
        )
        return
    alert_df = enrich_crypto_activity_display(pd.DataFrame(alerts))
    render_simple_trade_cards(
        friendly_crypto_trades(alert_df),
        ranked_lookup,
        "New Alerts",
        "No new alerts",
        "New watched-wallet buys above your alert size will show here.",
...
def render_simple_how_it_works() -> None:
    simple_page_header("?", "How It Works", "A plain-English guide for beginners.")
    st.markdown(
        """
        <div class="ww-card-grid">
          <div class="ww-trader-card"><div class="ww-whale-name">1. Find whales</div><div class="ww-buy-meta">WhaleWatch scans public wallet activity. It never sees private exchange accounts and never needs private keys.</div></div>
          <div class="ww-trader-card"><div class="ww-whale-name">2. Read the cards</div><div class="ww-buy-meta">Focus on Estimated Profit, Profitable Trade %, Recent Buys, and the tokens the wallet trades most.</div></div>
          <div class="ww-trader-card"><div class="ww-whale-name">3. Watch wallets</div><div class="ww-buy-meta">Save wallets you care about. Their tier and metrics stay stored in your private Firebase watchlist.</div></div>
          <div class="ww-trader-card"><div class="ww-whale-name">4. Track buys</div><div class="ww-buy-meta">Alerts show simple activity like Bought $24,500 of PEPE. Transfers and raw blockchain logs stay hidden.</div></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    st.info("WhaleWatch is read-only. It does not place trades, ask for seed phrases, or store trading credentials.")


def render_simple_sidebar_brand() -> None:
    logo_uri = logo_data_uri(str(LOGO_IMAGE_PATH), LOGO_IMAGE_PATH.stat().st_mtime_ns if LOGO_IMAGE_PATH.exists() else 0)
    img_html = f'<img src="{logo_uri}" alt="WhaleWatch logo">' if logo_uri else ""
    st.markdown(
        f"""
        <div class="ww-simple-brand">
          {img_html}
          <div>
            <div class="ww-simple-brand-title">WhaleWatch</div>
            <div class="ww-simple-brand-subtitle">Copy the whales</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_simple_whalewatch_app(settings_state: dict) -> None:
    ensure_crypto_watchlist_state()
    rows = st.session_state.get("crypto_rows", [])
    with st.sidebar:
        render_simple_sidebar_brand()
        page = st.radio("Navigation", ["Leaderboard", "Watchlist", "Alerts", "How It Works"], label_visibility="collapsed")
        st.divider()
        st.header("Find Whales")
        chain = st.selectbox("Chain", list(CHAIN_CONFIGS.keys()), key="simple-chain")
        token_options = ["All"] + list(CHAIN_CONFIGS[chain]["stable_tokens"].keys())
        token_filter = st.selectbox("Token", token_options, key="simple-token-filter")
        time_period_days = st.selectbox("Time period", [7, 30, 90], index=1, format_func=lambda days: f"Last {days} days", key="simple-days")
        min_transaction_size = st.number_input("Minimum buy size", min_value=0.0, value=25000.0, step=5000.0, key="simple-min-buy-size")
        max_wallets = st.number_input("Whales to scan", min_value=5, max_value=100, value=30, step=5, key="simple-max-wallets")
        discover_crypto = st.button("Find Whales", type="primary", key="simple-find-whales")
        alert_min = st.number_input(
            "Alert size",
            min_value=0.0,
            value=float(settings_state.get("min_trade_size", 250.0)),
            step=250.0,
            key="simple-alert-size",
        )
        update_user_settings(min_trade_size=float(alert_min))
        st.divider()
        st.caption(f"Logged in as {auth_session.get('email', 'unknown') if auth_session else 'unknown'}")
        if st.button("Sign Out", key="simple-sign-out"):
            sign_out()

    if discover_crypto:
        api_key = crypto_api_key(chain)
        if not api_key:
            st.error(f"{chain} scanning needs an explorer API key in Streamlit secrets.")
        else:
            try:
                with st.spinner("Finding whale wallets..."):
                    ...
                    rows = st.session_state["crypto_rows"]
                st.success("Whales loaded. Review the cards below.")
            except CryptoAPIError as exc:
                logger.warning("Simple crypto discovery failed", exc_info=True)
                st.error(f"Could not scan whales: {exc}")

    if page == "Leaderboard":
        render_simple_leaderboard(rows, chain, token_filter, int(time_period_days))
    elif page == "Watchlist":
        render_simple_watchlist(chain, token_filter, int(time_period_days), float(min_transaction_size), rows)
    elif page == "Alerts":
        render_simple_alerts(chain, token_filter, int(time_period_days), float(alert_min), rows)
    else:
        render_simple_how_it_works()
...
render_simple_whalewatch_app(settings_state)
st.stop()
